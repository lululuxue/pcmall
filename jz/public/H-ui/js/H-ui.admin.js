...
function open_iframe(title,url,w,h){
	...
	setTimeout(function(){
		// alert( $("#layui-layer1").attr('style'));
		var a1 = $("#layui-layer1").attr('style');
        var str=a1.replace(/top: -(\d)+.(\d)/g,"top:0");
        // alert(str)
        $("#layui-layer1").attr('style',str);
	},200)
}
 ...
 * @url 连接地址
 * */
function data_update(obj,url,ids){
    var ids = arguments[2] ? arguments[2] :'';
    if(ids=='')
    {
        //是否是批量删除
...
function sub_shop_all(name,name1){
    $.ajax({
        type:"POST",
        url: "/manager/welcome/sub_shop",
        data:{name:name},
        dataType:"json",
        success: function(data){
        	console.log(data);
            if(data.status!='y'){
                ...
                var str = '<option value="">--请选择分店--</option>';
                ...
                $("select[name=sub_shop]").html(str);
            }
        }
    });
}
function address_list($m_id){
    $.ajax({
        type:"get",
        url: "/admin/user/address_list/"+$m_id,
        dataType:"json",
        success: function(data){
            console.log(data);
            $('.chose-address .address-list').html(data.info)
