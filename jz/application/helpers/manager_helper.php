...
if (!function_exists('manager_login')) {
    function manager_login()
    {
        ...
        if (empty($admin_id)) {
            ...
            $admin_data = $CI->loop_model->get_id('admin', $admin_id, 'id,username,full_name,tel,role_id,status,job_no');
            if (empty($admin_data)) msg('用户信息不存在');

            //角色组
            if (!empty($admin_data['role_id'])) {
                $role = $CI->loop_model->get_where('role', array('id'=>$admin_data['role_id']));
