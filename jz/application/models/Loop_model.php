...
class Loop_model extends CI_Model
{
    ...
    public function insert($table, $data = FALSE, $insert_batch = '')
    {
        if ($data !== FALSE) {
            ...
            if (!empty($insert_id)) {
                return $insert_id;
            } else {echo $this->db->last_query()."<br>";
     ...
     * @return array
     */
    public function get_group_list($table, $data = array(), $limit = 0, $offset = 0, $groupby ,$orderby, $cache = '')
    {
        $is_cache = '';
        $result = array();
        //开启缓存
        if($cache!='') {
            $cache_name = md5('list'.$table.json_encode($data).$limit.$offset.$groupby);
            $result = cache('get',$cache_name);
            if (!empty($result)) {
                $is_cache = 'in';
                return $result;
