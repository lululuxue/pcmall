...
class Order_model extends CI_Model
{
    ...
    {file_put_contents('test.txt','64836578');
        //获取订单信息
        $this->load->model('loop_model');
        ...
        if ($order_data['payment_status'] == 1) {
            ...
            $price_real = $order_data['price_real'];
            $pay_money = $order_data['pay_money'];
            $update_data = array(
                'status'         => 5,
                'paytime'        => time(),
                'payment_status' => 1,
                'pay_money'     => $price_real
            );
...
//                'consume'     => $order_data['price_real'],
                'amount'     => ($order_data['service_id'] == 113 ? '+' : '-').number_format($price_real - $pay_money + $order_data['add_money'],1),
                'consume'     => ($order_data['service_id'] == 113 ? '+' : '-').number_format($price_real - $pay_money,1),
                'add_money'     => $order_data['add_money'],
                'addtime'    => time(),
                'payment_id' => 3,
                'note'       => $order_data['service_id'] == 113 ? '微信支付' : '微信交易',
                'admin_user' => ''
            );
            $this->db->trans_begin();
            ...
            $insert_data['type'] = 3;   //微信支付
            $insert_data['m_id'] = $order_data['m_id'];
            $insert_data['amount'] = $order_data['price_real'];
            $insert_data['addtime'] = time();
            $insert_data['note'] = $order_data['service_id'] == 113 ? '微信支付' : '微信交易';
            $insert_data['jifen'] = ceil(round((($price_real - $pay_money)/10)));
            $res1 = $this->loop_model->insert('jifen',$insert_data);
            //更新积分
            $jifen = $this->loop_model->get_id('user', $order_data['m_id']);
            $user_date['jifen'] = (int)$jifen['jifen'] + (int)(ceil(round(($price_real - $pay_money)/10)));
     ...
     * @return array
     */
    public function recharge($order_no,$transaction_id)
    {
        //数据验证
        if (empty($order_no)) {
            return '订单编号不能为空';
        }
		$order = $this->loop_model->get_where('order',array('order_no'=>$order_no));
		if($order['card_id'] && !$order['payment_no']){
            /*
           //根据id获取充值卡详情
           $card = $this->loop_model->get_where('card',array('Id'=>$order['card_id']));
           //根据用户id获取
           $user = $this->loop_model->get_where('user_card',array('m_id'=>$order['m_id']));


               $update_data['card_no']  = $user['card_no'];
               $update_data['card_id']   = (int)$order['card_id'];
                if($card['people_discount'] < $user['people_discount'] ) {
                    $update_data['people_discount'] = $card['people_discount'];
                    $update_data['care_discount'] = $card['people_discount'];
                    $update_data['time']              = $card['time'];
                    $update_data['is_member_card']     = $card['is_card'] == 1 ? 1 : $user['is_member_card'];
                    $update_data['endtime']            = strtotime($card['time']." month");
                    $update_data['all']                = $card['count'];
                }
               $update_data['count']				 = $user['count']+$card['count'];
               $update_data['care_count']         = $user['care_count']+$card['care_count'];
               $update_data['total_count']        = $user['total_count']+$card['count'];
               $update_data['addtime']            = time();
               $update_data['add_money']          = $user['add_money'];
               $update_data['add_money_su']       = $user['add_money_su'];
               $update_data['total_care_count']   = $user['total_care_count'];



           $where = array('m_id'=>$order['m_id']);

           $res = $this->loop_model->update_where('user_card',$update_data,$where);
           $this->loop_model->update_where('order', array('payment_no' => $transaction_id), array('order_no' => $order_no));
           if($res){
               return 'y';
           }else{
               return '信息保存失败';
            ...
            */
            ...
            //根据id获取充值卡详情
            $card = $this->loop_model->get_where('new_card',array('Id'=>$order['card_id']));
            //根据用户id获取
            $user = $this->loop_model->get_where('user_new_card',array('m_id'=>$order['m_id']));

            //$update_data['card_no']  = $user['card_no'];
            //$update_data['card_id']   = (int)$order['card_id'];

            $update_data['count']				 = $user['count']+$card['count'];
            $update_data['add_money']          = $user['add_money']+$card['add_money'];
            $update_data['total_count']       = $user['total_count']+$card['count']+$card['add_money'];

            $where = array('m_id'=>$order['m_id']);

            $res = $this->loop_model->update_where('user_new_card',$update_data,$where);
            $this->loop_model->update_where('order', array('payment_no' => $transaction_id), array('order_no' => $order_no));

         }
        if($res){
		    if($order['service_id'] == 113 ){
                //发送短信通知用户充值了多少钱
                $mobile = $user['card_no'];
                $tmp = 'SMS_147436077';
                $this->load->library('SmsDemo');
                $res  = SmsDemo::sendOtherSms($mobile,$tmp,'',$card['count'],'');
                if ($res->Code == 'OK') {
                    return 'y';
                } else {
                    return($res->Message);
