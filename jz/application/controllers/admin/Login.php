...
class Login extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($admin_id)) {
            header('location:' . site_url('/admin/welcome'));
        } else {
            $redirect_url = trim($this->input->get('redirect_url', true));//返回链接
            if (empty($redirect_url)) $redirect_url = site_url('/admin/welcome');
    ...
    public function login_act()
    {
        ...
        if (!empty($username) && !empty($password)) {
            ...
            if ($admin_data['username'] == '') {
                ...
                error_json('密码错误');
            } elseif ($admin_data['command'] != $command) {
                error_json('口令错误');
    ...
    public function loginout()
    {
        ...
        echo "<script>alert('您已经退出登陆。');parent.window.location.href='" . site_url('/admin/login') . "'</script>";
    }
