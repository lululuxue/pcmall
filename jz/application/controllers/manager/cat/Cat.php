...
class Cat extends CI_Controller
{
    ...
    {
        //查到数据
        $list = $this->loop_model->get_list('card','','','','id asc');//列表
        assign('list', $list);//print_r($list);
    ...
    public function edit_recharge($id)
    {
        $id = (int)$id;
        if (!empty($id)) {
            $item               = $this->loop_model->get_id('card', $id);
            assign('item', $item);
        }
        $this->load->helpers('upload_helper');//加载上传文件插件
    ...
    public function save_recharge(){
        if (is_post()) {
            ...
            $update_data = array(
                ...
                'time'         => $data_post['time'],
                'people_discount'=> $data_post['people_discount'],
                'care_discount'  => $data_post['care_discount'],
                'sortnum'     => (int)$data_post['sortnum']
            );
            if (!empty($data_post['Id'])) {
                //修改数据
                ...
                $res = $this->loop_model->insert('card', $update_data);
            }
            if (!empty($res)) {
                error_json('y');
            } else {
    ...
    public function delete_recharge()
    {
        $id = $this->input->post('id', true);
        if (empty($id)) error_json('id错误');
        $res = $this->loop_model->delete_id('card', $id);
        if (!empty($res)) {
