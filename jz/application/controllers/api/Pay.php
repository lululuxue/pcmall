    ...
    public function do_pay()
    {
        ...
        if (!empty($order_no) ) {
            ...
            foreach ($all_order_no as $key) {
                $where['order_no'] = $key;
                $where['status <'] = 4;
                $order_data = $this->loop_model->get_where('order',$where);
                if (!empty($order_data)) {
                    $order_no_data[] = $order_data['order_no'];
					//$order_price = 1;
                    $price_real = round(floatval($order_data['price_real']*100));
                    $pay_money = round(floatval($order_data['pay_money']*100));
                    $order_price     =  $order_price + $price_real - $pay_money;//支付金额
                    ...
                    foreach ($order_no as $key) {
                        ...
                        if ($order_res == 'y') {
                            ...
                            if (!empty($pay_res['transaction_id'])) {
								...
								$this->order_model->recharge($key,$pay_res['transaction_id']);
