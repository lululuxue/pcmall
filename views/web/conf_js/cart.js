...
//***************************************************************************************
$(function(){
    $('#shop_list').on('change','.coupons_list',function(){
        sum_order_price();
    })
    $('#shop_list').on('change','.delivery_list',function(){
        sum_order_price();
    })
})
//开始计算所有费用
function sum_order_price() {
    ...
    $('.shop_view').each(function () {
        ...
        //运费价格
        delivery_price = parseFloat($(this).find('.delivery_list').find('option:selected').attr('deliveryprice'));
        //优惠券价格
        coupons_amount_num = $(this).find('.coupons_list').find('option:selected').attr('couponsamount');
