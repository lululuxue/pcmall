...
function add_order() {
    ...
    if (check_sku_num() === true) {
        ...
        window.location.href = '/cart/confirm?sku_id=' + sku_id + '&buy_num=' + buy_num + '&buy_type=sku';
    }
}
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status == 'y') {
                    ...
                    if (data.info == '请登录后操作') {
                        layer.msg('正在跳转登录');
                        window.parent.parent.location.href = '/welcome/login?redirect_url=' + window.location.href;
                    } else {
                        setTimeout(function () {
                            //隐藏选择框
                            $('.masker').removeClass('maskershow');
...
function favorite_delete(id, goods_id) {
    if (id == '') {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status == 'y') {
                    $('#godos_favorite_' + goods_id).text('收藏商品');
                    layer.msg('收藏取消');
                } else {
                    if (data.info == '请登录后操作') {
                        layer.msg('正在跳转登录');
...
//点击图片规格改变主图
function set_sku_image(picurl) {
    $('#sku_image').attr('src', picurl);
}
