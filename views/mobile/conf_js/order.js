...
//用户删除订单
function order_delete(order_id, source){
    if (order_id=='') {
        layer.msg('订单ID错误');return false;
    }
    $.ajax({
        ...
        success: function(data){
            if (data.status=='y') {
                ...
                $.each(data.result, function(index, obj){
                    html += '<li><label for="payment_'+obj.id+'"><div class="sellerbox flexbox"><div class="radiobox"> <input';
                    if (payment_id==obj.id) {
                        html += ' checked';
                    }
                    html += ' class="inputradio" type="radio" name="payment_id" value="'+obj.id+'" id="payment_'+obj.id+'"/> <span></span> </div><div class="sellerN">'+obj.name+'</div>  </div> </label></li>';
                })
                html += '</div>'
                //支付方式展示
                layer.open({
