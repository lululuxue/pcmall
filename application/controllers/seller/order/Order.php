...
class Order extends CI_Controller
{
    ...
    public function edit($id = '')
    {
        $id = (int)$id;
        if (!empty($id)) {
            $item                        = $this->loop_model->get_where('order', array('id' => $id, 'shop_id' => $this->shop_id));
    ...
    public function save()
    {
        if (is_post()) {
            ...
            $item      = $this->loop_model->get_where('order', array('id' => $data_post['id'], 'shop_id' => $this->shop_id));
            if ($item['status'] > 1) {
                ...
                if (!empty($res)) {
                    ...
                    $order_log_data = array(
                        ...
                        'admin_user' => $this->shop_data['username'],
                        'action'     => '编辑',
    ...
    public function delete_recycle()
    {
        ...
        $res          = $this->loop_model->update_where('order', array('is_del' => 1), $update_where);
        if (!empty($res)) {
            if (is_array($id)) $id = join(',', $id);
            shop_admin_log_insert('删除订单到回收站' . $id);
            error_json('y');
        } else {
            error_json('删除失败');
        }
    ...
    public function reduction_recycle()
    {
        ...
        $res          = $this->loop_model->update_where('order', array('is_del' => 0), $update_where);
        if (!empty($res)) {
            if (is_array($id)) $id = join(',', $id);
            shop_admin_log_insert('还原订单' . $id);
            error_json('y');
        } else {
            error_json('还原失败');
        }
    ...
    public function view($id = '')
    {
        $id = (int)$id;
        if (!empty($id)) {
            $order_data                        = $this->loop_model->get_where('order', array('id' => $id, 'shop_id' => $this->shop_id));
    ...
    public function order_send_save()
    {
        ...
        if (!empty($id)) {
            ...
            if (!empty($order_data)) {
                ...
                $this->load->model('order/order_model');
                $up_res = $this->order_model->send($id, $express_company_id, $delivery_code, $this->shop_data['username'], $note);
                error_json($up_res);
            }
        }
    }
    ...
    public function refund_save()
    {
        ...
        if (!empty($sku_id)) {
            ...
            //订单信息
            $order_data = $this->loop_model->get_where('order', array('id' => $order_sku_data['order_id'], 'shop_id' => $this->shop_id));
            if (is_refund($order_data)) {
                if ($order_sku_data['is_refund'] == 1) {
                    ...
                    $this->load->model('order/order_model');
                    $doc_id = $this->order_model->refund_add($refund_doc_data, $this->shop_data['username']);
                    if (!empty($doc_id)) {
                        //开始处理退款
                        $this->order_model->refund_confirm($doc_id, $this->shop_data['username'], $desc, 1);
                        error_json('y');
                    } else {
                        error_json('申请失败');
                    }
    ...
    public function print_distribution()
    {
        ...
        if (!empty($id)) {
            $where_data = array(
                ...
                'where'    => array('o.shop_id' => $this->shop_id),
                'join'     => array(
