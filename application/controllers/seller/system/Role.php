...
class Role extends CI_Controller
{
    ...
    public function save()
    {
        if (is_post()) {
            ...
            if (!empty($data_post['id'])) {
                //修改数据
                $res = $this->loop_model->update_where('role', $update_data, array('id' => $data_post['id'], 'shop_id' => $this->shop_id));
                shop_admin_log_insert('修改角色' . $data_post['id']);
            } else {
                //增加数据
                $update_data['shop_id'] = $this->shop_id;
                $res                    = $this->loop_model->insert('role', $update_data);
                shop_admin_log_insert('增加角色' . $res);
            }
            if (!empty($res)) {
                error_json('y');
            } else {
    ...
    public function delete_recycle()
    {
        ...
        $res          = $this->loop_model->update_where('role', array('status' => 1), $update_where);
        if (!empty($res)) {
            if (is_array($id)) $id = join(',', $id);
            shop_admin_log_insert('删除角色到回收站' . $id);
            error_json('y');
        } else {
            error_json('删除失败');
        }
    ...
    public function reduction_recycle()
    {
        ...
        $res          = $this->loop_model->update_where('role', array('status' => 0), $update_where);
        if (!empty($res)) {
            if (is_array($id)) $id = join(',', $id);
            shop_admin_log_insert('还原角色' . $id);
            error_json('y');
        } else {
            error_json('还原失败');
        }
    ...
    public function delete()
    {
        ...
        $res          = $this->loop_model->delete_where('role', $delete_where);
        if (!empty($res)) {
            shop_admin_log_insert('删除角色' . $id);
            error_json('y');
        } else {
            error_json('删除失败');
        }
