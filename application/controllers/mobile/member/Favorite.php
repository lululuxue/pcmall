...
class Favorite extends CI_Controller
{
    ...
    {/*
        ...
        //搜索条件end
        $where_data['select'] = 's.id,s.name,s.logo,f.id as concern_id';
        $where_data['where']  = array('f.m_id' => $this->member_data['id']);//过滤用户
        $where_data['join']   = array(
            array('goods_brand as s', 's.id=f.brand_id')
        );
        //查到数据
        $list = $this->loop_model->get_list('member_concern_brand as f', $where_data, $pagesize, $pagesize * ($page - 1), 'f.add_time desc');//列表
        assign('list', $list);
        //开始分页start
        $all_rows = $this->loop_model->get_list_num('member_concern_brand as f', $where_data);//所有数量;
        assign('page_count', ceil($all_rows / $pagesize));
        //开始分页end
        assign('list', $list);
*/
        display('/member/favorite/brand.html');
    }
