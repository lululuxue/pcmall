...
class Coupons extends CI_Controller
{
    ...
    public function save()
    {
        if (is_post()) {
            ...
            if (!empty($data_post['id'])) {
                ...
                admin_log_insert('修改优惠券活动' . $data_post['id']);
            } else {
                //增加数据
                $res = $this->loop_model->insert('coupons', $update_data);
                admin_log_insert('增加优惠券活动' . $res);
            }
            if (!empty($res)) {
                error_json('y');
            } else {
    ...
    public function detail()
    {
        ...
        //$where_data['where']['shop_id'] = $this->shop_id;过滤店铺
        $search_where = array(
            'status'   => $status,
            'is_send'  => $is_send,
            'is_close' => $is_close,
    ...
    public function detail_set_user($id)
    {
        ...
        if (!empty($member_data)) {
            ...
            if (!empty($coupons_detail_data) && $coupons_detail_data['m_id'] == 0) {
                ...
                if (!empty($res)) {
                    admin_log_insert('给用户' . $username . '绑定优惠券' . $id);
                    error_json('y');
                } else {
                    error_json('绑定失败');
                }
