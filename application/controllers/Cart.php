...
class Cart extends CI_Controller
{
    ...
    public function confirm()
    {
        ...
        if (!empty($sku_id) && !empty($buy_type)) {
            ...
            $address_url = site_url('/member/address?redirect_url=' . urlencode(get_now_url()));
            ...
            if (is_array($shop_list)) {
                $redirect_url = preg_replace('/&address_id=(\d+)/', '', get_now_url());
    ...
    public function commit()
    {
        ...
        if ($all_order_price <= 0) {
            header('location:' . site_url('/member/order'));
        } else {
            //直接去支付
            header('location:' . site_url('/api/pay/do_pay?client=&order_no=' . join(',', $order_no)));
        }
    }
